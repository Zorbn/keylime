...
test_doc!(search_forward, HELLO_GOODBYE_TEXT, |ctx, doc| {
    let position = doc.search("world", doc.get_line_end(0), false, ctx.gfx);
    assert_eq!(position, Some(Position::new(8, 1)));
});

test_doc!(search_forward_wrap, HELLO_GOODBYE_TEXT, |ctx, doc| {
...
test_doc!(search_backward, HELLO_GOODBYE_TEXT, |ctx, doc| {
    let position = doc.search("world", Position::new(0, 1), true, ctx.gfx);
    assert_eq!(position, Some(Position::new(6, 0)));
});

test_doc!(search_backward_wrap, HELLO_GOODBYE_TEXT, |ctx, doc| {
