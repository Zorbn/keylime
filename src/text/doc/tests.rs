...
test_with_doc!(search_forward, HELLO_GOODBYE_TEXT, |ctx, doc| {
    let position = doc.search("world", doc.line_end(0), false, ctx.config, ctx.gfx);
    assert_eq!(position, Some(Position::new(8, 1)));
});

test_with_doc!(search_forward_wrap, HELLO_GOODBYE_TEXT, |ctx, doc| {
    let position = doc.search("hello", Position::new(0, 1), false, ctx.config, ctx.gfx);
...
test_with_doc!(
    ...
    HELLO_GOODBYE_TEXT,
    |ctx, doc| {
        let position = doc.search_forward("hello", Position::new(0, 1), false, ctx.config, ctx.gfx);
        assert_eq!(position, None);
    }
);

test_with_doc!(search_backward, HELLO_GOODBYE_TEXT, |ctx, doc| {
    let position = doc.search("world", Position::new(0, 1), true, ctx.config, ctx.gfx);
    assert_eq!(position, Some(Position::new(6, 0)));
});

test_with_doc!(search_backward_wrap, HELLO_GOODBYE_TEXT, |ctx, doc| {
    let position = doc.search("goodbye", Position::new(0, 1), true, ctx.config, ctx.gfx);
...
test_with_doc!(
    ...
    |ctx, doc| {
        let position =
            doc.search_backward("goodbye", Position::new(0, 1), false, ctx.config, ctx.gfx);
        assert_eq!(position, None);
    }
);

test_with_doc!(repeated_search_forward, HELLO_GOODBYE_TEXT, |ctx, doc| {
    let position = doc.search("world", Position::ZERO, false, ctx.config, ctx.gfx);
    assert_eq!(position, Some(Position::new(6, 0)));

    let position = doc.search("world", position.unwrap(), false, ctx.config, ctx.gfx);
    assert_eq!(position, Some(Position::new(8, 1)));
...
test_with_doc!(repeated_search_backward, HELLO_GOODBYE_TEXT, |ctx, doc| {
    let position = doc.search("world", Position::ZERO, true, ctx.config, ctx.gfx);
    assert_eq!(position, Some(Position::new(8, 1)));

    let position = doc.search("world", position.unwrap(), true, ctx.config, ctx.gfx);
    assert_eq!(position, Some(Position::new(6, 0)));
