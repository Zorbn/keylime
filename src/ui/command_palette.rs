...
impl CommandPalette {
    ...
    pub fn update(&mut self, editor: &mut Editor, ctx: &mut Ctx) {
        ctx.ui.begin_container(
            WidgetLayout {
                // TODO: Use real position here
                position: Some(VisualPosition::new(0.0, 0.0)),
                ..Default::default()
            },
        ...
        let result_input = self.result_list.update(ctx, true, true);
        ...
        match result_input {
            ...
            ResultListInput::Close => self.close(),
        }

        self.tab.update(None, &mut self.doc, ctx);
        self.update_results(editor, ctx);

        if self.is_open && !ctx.ui.is_focused() {
            self.close();
        }
    ...
    fn submit(&mut self, kind: ResultListSubmitKind, editor: &mut Editor, ctx: &mut Ctx) {
        ...
        match action {
            ...
            CommandPaletteAction::Close => self.close(),
        }
    }
